...
static struct mm_region ss528v100_mem_map[] = {
	{
		.virt = 0x0UL,
		.phys = 0x0UL,
		.size = 0x40000000UL,
...
struct mm_region *mem_map = ss528v100_mem_map;
 ...
 */
void *filling_space(void *s, int c, size_t count)
{
...
u64 get_tcr(int el, u64 *pips, u64 *pva_bits)
{
	...
	if (max_addr > (1ULL << 44)) {
		ips = 5; /* 5 : Intermediate Physical Address Size */
		va_bits = 48; /* 48 : Virtual address bit */
	} else  if (max_addr > (1ULL << 42)) {
		ips = 4; /* 4 : Intermediate Physical Address Size */
		va_bits = 44; /* 44 : Virtual address bit */
	} else  if (max_addr > (1ULL << 40)) {
		ips = 3; /* 3 : Intermediate Physical Address Size */
		va_bits = 42; /* 42 : Virtual address bit */
	} else  if (max_addr > (1ULL << 36)) {
		ips = 2; /* 2 : Intermediate Physical Address Size */
		va_bits = 40; /* 40 : Virtual address bit */
	} else  if (max_addr > (1ULL << 32)) {
		ips = 1; /* 1 : Intermediate Physical Address Size */
		va_bits = 36; /* 36 : Virtual address bit */
	} else {
		ips = 0;
		va_bits = 32; /* 32 : Virtual address bit */
	}
...
static int pte_type(u64* const pte)
{
...
{
	/* Page is 12 bits wide, every level translates 9 bits; Level 3 cache */
	return (12 + 9 * (3 - level));
}
...
static u64 *create_table(void)
{
	...
	if (gd->arch.tlb_fillptr - gd->arch.tlb_addr > gd->arch.tlb_size) {
		uart_early_puts("### ERROR ### Please increase the size in get_page_table_size() ###\n");
		for (;;)
			;
	}

	/* Mark all entries as invalid */
	filling_space(new_table, 0, pt_len);

	return new_table;
}

static void set_pte_table(u64* const pte, u64* const table)
{
...
static void split_block(u64* const pte, int level)
{
	u64 old_pte = *pte;
	u64 *new_table = NULL;
	u64 i;
	...
	if (pte_type(pte) != PTE_TYPE_BLOCK) {
		uart_early_puts("### ERROR ### Some driver code wants to modify dcache settings for an range not covered in mem_map.###\n");
		for (;;)
			;
	}
...
static void add_map(struct mm_region* const map)
{
	u64 *pte = NULL;
	u64 virt = map->virt;
	u64 phys = map->phys;
	...
	while (size) {
		...
		for (level = 1; level < 4; level++) {
			...
			if (!pte) {
				uart_early_puts("### ERROR ### pte not found ###\n");
				for (;;)
					;
			}
...
void setup_pgtables(void)
{
	...
	if (!gd->arch.tlb_fillptr || !gd->arch.tlb_addr) {
		uart_early_puts("### ERROR ### Page table pointer not setup. ###\n");
		for (;;)
			;
	}
...
static void early_mmu_setup(void)
{
	unsigned int el = current_el();

	/* set gd addr for early mmu setup */
	asm("mov x18, %0"::"r"(EARLY_GD_ADDR));

	/* global data is already setup, no allocation yet */
	gd->arch.tlb_addr = EARLY_PAGE_TBL_ADDR;
	gd->arch.tlb_fillptr = gd->arch.tlb_addr;
	gd->arch.tlb_size = EARLY_PAGE_TBL_SIZE;
...
u64 *__weak arch_get_page_table(void) {
	uart_early_puts("No page table offset defined\n");
...
static u64 set_one_region(u64 start, u64 size, u64 attrs, int level)
{
	...
	if (pte_type(pte) != PTE_TYPE_TABLE) {
		uart_early_puts("### ERROR ### PTE should be a table. ###\n");
		for (;;)
			;
	}
