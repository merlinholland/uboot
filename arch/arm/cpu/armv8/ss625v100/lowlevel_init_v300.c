...
#define HPM_CORE_MIN	200
#define HPM_CORE_MAX    300

#define HPM_CPU_MIN   240
#define HPM_CPU_MAX   330
...
static unsigned hpm_value_avg(unsigned int* const val, unsigned int num)
{
	unsigned int i;
	unsigned tmp = 0;
...
#define calc_pcore_mv_formula(hpm_core) (1330 - 1.533 * (hpm_core))
#define calc_core_delta_mv_formula(hpm_core, temperature_delta) (((((double)(hpm_core) - 200) / 3.3) + 30.0) * \
								((double)(temperature_delta) / 85.0))
#define calc_pcpu_mv_formula(hpm_cpu) (1728.384 - 2.545 * (hpm_cpu))
...
static void start_svb(double *pcore_mv, double *pcpu_mv)
{
	...
	*pcore_mv = calc_pcore_mv_formula(hpm_core);
	...
	if (temperature_delta > TEMPERATURE_DELTA_MAX)
		temperature_delta = TEMPERATURE_DELTA_MAX;
	core_delta_mv = calc_core_delta_mv_formula(hpm_core, temperature_delta);

	*pcpu_mv = calc_pcpu_mv_formula(hpm_cpu); /* svb001 */
