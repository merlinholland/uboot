...
static u32 phase_cfg[][PHASE_TYPE_NUM] = {
	...
	[SD_HS]      = { 16, 4 },
	[MMC_HS_200]  = { 22, 0 },
#if defined(CONFIG_TARGET_SS524V100) || defined(CONFIG_TARGET_SS615V100) || \
	defined(CONFIG_TARGET_SS522V101) || defined(CONFIG_TARGET_SS522V100)
	[MMC_HS_400]  = {  9, 0 },
	[MMC_HS_400_ES]  = {  9, 0 }
#endif
};
...
static void mmc_pin_mux_config(int is_emmc)
{
	...
	set_pin_mux(REG_MMC_CLK_IO, pin_mux);
	set_pin_mux(REG_MMC_CMD_IO, pin_mux);
	for (i = 0; i < bus_width; i++)
		set_pin_mux(reg_data_io[i], pin_mux);
	/* REG_MMC_RST_IO & SDIO_CARD_POWER_EN_N is the same pin */
	set_pin_mux(REG_MMC_RST_IO, pin_mux);
