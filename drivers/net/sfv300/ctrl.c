...
static inline u32 _bspeth_irq_enable(struct bspeth_netdev_local const *ld, u32 irqs)
{
...
static inline u32 _bspeth_irq_disable(struct bspeth_netdev_local const *ld, u32 irqs)
{
	u32 old = bspeth_readl(ld, GLB_RW_IRQ_ENA);
...
u32 bspeth_readl(struct bspeth_netdev_local const *ld, u32 ofs)
{
	...
	reg = _readl((uintptr_t)(ld->iobase_phys + ofs));
	bspeth_trace(ETH_TRACE_ETH, "_readl(0x%08X) = 0x%08X",
...
void bspeth_writel(struct bspeth_netdev_local const *ld, u32 v, u32 ofs)
{
	...
	bspeth_trace(ETH_TRACE_ETH, "_writel(0x%08X) = 0x%08X",
