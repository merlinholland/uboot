...
#define ETH_BIG_ENDIAN	0
#define ETH_LITTLE_ENDIAN	3
...
/* IRQs bit name */
#define ETH_INT_RX_RDY_U      bit(0)
#define ETH_INT_RX_RDY_D      bit(20)
#define ETH_INT_TX_FIN_U		bit(1)
#define ETH_INT_TX_FIN_D		bit(21)
#define ETH_INT_LINK_CH_U		bit(2)
#define ETH_INT_LINK_CH_D		bit(22)
#define ETH_INT_SPEED_CH_U	bit(3)
#define ETH_INT_SPEED_CH_D	bit(23)
#define ETH_INT_DUPLEX_CH_U	bit(4)
#define ETH_INT_DUPLEX_CH_D	bit(24)
#define ETH_INT_STATE_CH_U	bit(5)
#define ETH_INT_STATE_CH_D	bit(25)
#define ETH_INT_TXQUE_RDY_U	bit(6)
#define ETH_INT_TXQUE_RDY_D	bit(26)
#define ETH_INT_MULTI_RXRDY_U	bit(7)
#define ETH_INT_MULTI_RXRDY_D	bit(27)

#define ETH_INT_MDIO_FINISH	bit(12)
#define ETH_INT_UNKNOW_VLANID	bit(13)
#define ETH_INT_UNKNOW_VLANM	bit(14)
...
#define is_recv_packet(ld)		(bspeth_readl(ld, GLB_RW_IRQ_RAW) & (ud_bit_name(ETH_INT_RX_RDY)))
#define is_recv_packet_rx(ld)	((bspeth_readl(ld, ud_reg_name(GLB_RO_QUEUE_STAT)) >> 8) & 0x3F)
#define hw_set_rxpkg_finish(ld)	bspeth_writel(ld, ud_bit_name(ETH_INT_RX_RDY), GLB_RW_IRQ_RAW)
...
u32 bspeth_irq_enable(struct bspeth_netdev_local const *ld, u32 irqs);
u32 bspeth_irq_disable(struct bspeth_netdev_local const *ld, u32 irqs);
/* return irqstatus */
...
u32 bspeth_set_endian_mode(struct bspeth_netdev_local const *ld, u32 mode);
...
#define bspeth_invalid_rxpkg_len(len)	(!((len) >= 42 && (len) <= ETH_MAX_FRAME_SIZE))
