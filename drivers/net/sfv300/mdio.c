...
static void mdio_reg_reset(struct bspeth_netdev_local const *ld)
{
...
static int wait_mdio_ready(struct bspeth_netdev_local* const ld)
{
	int timeout_us = 5000;
...
int bspeth_mdio_read(struct bspeth_netdev_local* const ld, int phy_addr, unsigned int regnum)
{
	int val = 0;
...
error_exit:
	...
	bspeth_trace(ETH_TRACE_MDIO, "phy_addr = %d, regnum = %d, val = 0x%04x", phy_addr,
				(int)regnum, val);

	return val;
}

int bspeth_mdio_write(struct bspeth_netdev_local* const ld, int phy_addr, int regnum, int val)
{
	if (ld == NULL)
		return -1;
	...
	bspeth_trace(ETH_TRACE_MDIO, "phy_addr = %d, regnum = %d", phy_addr, regnum);
...
int bspeth_mdio_reset(struct bspeth_netdev_local const *ld)
{
	mdio_reg_reset(ld);
...
int bspeth_mdio_init(struct bspeth_netdev_local const *ld)
{
	local_lock_init(ld);
...
void bspeth_mdio_exit(struct bspeth_netdev_local const *ld)
{
	local_lock_exit(ld);
}
