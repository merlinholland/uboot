...
struct bspeth_netdev_local bspeth_devs_priv[MAX_PORT] = {
	{
		...
		.phy_addr = SFV_PHY_U,
		.phy_intf = ETH_MII_RMII_MODE_U,
	},
	{
		.port = DOWN_PORT,
		.mii_name = mdio_bus_name,
		.phy_addr = SFV_PHY_D,
		.phy_intf = ETH_MII_RMII_MODE_D,
	},
};
...
static int bspeth_init_hw_desc_queue(struct bspeth_netdev_local *ld)
{
	...
	const int size = ETH_HW_DESC_DEPTH * sizeof(struct bspeth_frame_desc);
	/* init rx fq */
...
#ifdef ETH_RX_QUEUE_MULTI_DESC
static int bspeth_destroy_hw_desc_queue(struct bspeth_netdev_local* const ld)
{
...
int eth_set_host_mac_address(struct eth_device* const dev)
{
	...
	if (memset_s(mac, sizeof(mac), 0, sizeof(mac)))
		printf("memset_s failed!\n");
...
static void phy_print_status(struct bspeth_netdev_local const *ld, unsigned int stat)
{
	printf("%s : phy status change : LINK=%s : DUPLEX=%s : SPEED=%s\n",
		   (ld->port == UP_PORT) ? "eth0" : "eth1",
		   (stat & ETH_LINKED) ? "UP" : "DOWN",
		   (stat & ETH_DUP_FULL) ? "FULL" : "HALF",
		   (stat & ETH_SPD_100M) ? "100M" : "10M");
}

static void bspeth_adjust_link(struct bspeth_netdev_local* const ld)
{
	u32 stat = 0;
	int timeout_us = 1000;
...
retry:
	...
	stat |= miiphy_link(ld->mii_name, ld->phy_addr) ? ETH_LINKED : 0;
	stat |= miiphy_duplex(ld->mii_name, ld->phy_addr) == FULL ? ETH_DUP_FULL : 0;
	stat |= miiphy_speed(ld->mii_name, ld->phy_addr) == _100BASET ? ETH_SPD_100M : 0;
	if (--timeout_us && !(stat & ETH_LINKED))
		goto retry;
...
static int bspeth_net_open(struct bspeth_netdev_local *ld)
{
	...
	bspeth_writel_bits(ld, ETH_HW_TXQ_DEPTH, ud_reg_name(GLB_QLEN_SET), BITS_TXQ_DEP);

	/* setup hardware rx dep */
	bspeth_writel_bits(ld, ETH_HW_RXQ_DEPTH, ud_reg_name(GLB_QLEN_SET), BITS_RXQ_DEP);

	ld->link_stat = 0;
	bspeth_adjust_link(ld);

	bspeth_irq_enable(ld, ud_bit_name(ETH_INT_RX_RDY));
...
static int bspeth_dev_remove(struct bspeth_netdev_local* const ld)
{
#ifdef ETH_RX_QUEUE_MULTI_DESC
...
{
	struct bspeth_netdev_local *ld = NULL;
	int ret;
	int count = 30;
	...
	ld = (struct bspeth_netdev_local *)dev->priv;
	...
	bspeth_set_crg_phy_mode(ld->phy_intf);
#endif
	...
	if (ld->mii_name == NULL ||
	    phy_detected(ld->mii_name, ld->phy_addr) == false) {
		goto _error_bspeth_init;
	}
...
static int bspeth_recv(struct eth_device *dev)
{
	...
	if (!recvq_ready)
		bspeth_trace(ETH_TRACE_DRV, "hw can't add a rx addr.");

	while (recvq_ready &&
		   ((hw_offset + 1) % ETH_HW_DESC_DEPTH != rec_offset)) {
		...
		hw_offset = (hw_offset + 1) % ETH_HW_DESC_DEPTH;
	...
	if (is_recv_packet_rx(ld)) {
		...
		rec_offset = (rec_offset + 1) % ETH_HW_DESC_DEPTH;
		...
		if (memcpy_s((void *)net_rx_packets[0], sizeof(net_rx_packets[0]), (void *)receive_fd.frm_addr,
					 receive_fd.frm_len))
			printf("memcpy_s failed!\n");
		...
		bspeth_trace(ETH_TRACE_DRV, "hw rx timeout.");
	}
...
static int bspeth_send(struct eth_device *dev, void *packet, int length)
{
	...
	/* enable tx int */
	bspeth_irq_enable(ld, ud_bit_name(ETH_INT_TXQUE_RDY));
	...
	} while (--timeout_us && !(ints & ud_bit_name(ETH_INT_TXQUE_RDY)));
...
static int bspeth_register_dev(unsigned char port_id)
{
	...
	if (snprintf_s(dev->name, sizeof(dev->name), sizeof(dev->name) - 1, "eth%d", port_id) < 0)
